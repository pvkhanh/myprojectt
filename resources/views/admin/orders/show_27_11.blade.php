...
@push('styles')
    <style>
        ...
        /* Gradient màu cho từng trạng thái */
                    ...
                    <div class="card-body">
                        @if ($payment)
                            ...
                            @if ($payment->needsVerification())
                                <div class="alert alert-warning payment-alert mb-3 d-flex align-items-center">
                                ...
                                <div>
                                    @php
                                        $statusColors = [
                                            ...
                                            'processing' => 'bg-info text-white', // Đang xử lý
                                            'paid' => 'bg-success text-white', // Đã thanh toán
                                            'failed' => 'bg-danger text-white', // Thanh toán thất bại
                                        ];
                                        $statusClass =
                                            $statusColors[$payment->status->value] ?? 'bg-secondary text-white';
                                    @endphp
                                ...
                                <div class="mb-3">
                                    ...
                                    <div>
                                        @if ($payment->is_verified)
                                            <span class="badge bg-success text-white rounded-pill px-3 py-2 mb-2">
                                            ...
                                            @endif
                                        @else
                                            <span class="badge bg-secondary text-white rounded-pill px-3 py-2 mb-2">
...
<style>
    ...
    .timeline-step::before {
        content: ''; position: absolute; top: 40px; left: 50%; right: -50%;
    ...
    .timeline-icon {
        ...
        background: #e0e0e0; border: 5px solid #fff;
        box-shadow: 0 4px 15px rgba(0,0,0,0.1); transition: all 0.3s ease;
    }
    .timeline-step.active .timeline-icon {
        animation: pulse 2s infinite; box-shadow: 0 6px 25px rgba(102,126,234,0.4);
    }
    .timeline-step.completed::before { background: linear-gradient(to right, #28a745, #20c997); }
    .timeline-step.completed .timeline-icon { background: linear-gradient(135deg, #28a745, #20c997); }
    .timeline-step.active .timeline-icon {
        background: linear-gradient(135deg, #667eea, #764ba2); transform: scale(1.1);
    }
    @keyframes pulse { 0%, 100% { transform: scale(1.1); } 50% { transform: scale(1.15); } }
    .action-card { transition: transform 0.2s; }
    .action-card:hover { transform: translateY(-3px); }
    .payment-alert { border-left: 4px solid #ffc107; animation: glow 2s ease-in-out infinite; }
    @keyframes glow { 0%, 100% { box-shadow: 0 0 5px rgba(255,193,7,0.3); } 50% { box-shadow: 0 0 20px rgba(255,193,7,0.6); } }
    .status-badge { font-size: 0.95rem; padding: 8px 16px; }
    ...
    .readonly-badge { background: #e9ecef; color: #495057; padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; }
</style>
@endpush
...
<div class="container-fluid px-4">
                ...
                <div>
                    <h2 class="fw-bold text-dark mb-2">
                        ...
                        <span class="readonly-badge ms-2">
                            <i class="fa-solid fa-eye me-1"></i>Chế độ xem
                        </span>
                    </h2>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
    ...
    <div class="row g-4">
        <!-- Left Column - Order Details (Read-Only) -->
        <div class="col-lg-8">
            <!-- Order Items -->
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white border-bottom py-3">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="info-value">{{ $order->shippingAddress->receiver_name }}</div>
                        </div>
                        <div class="col-md-6">
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    ...
                    @if ($order->admin_note)
                    <div>
                        <div class="info-label">Ghi chú nội bộ (Admin):</div>
                        <div class="p-3 bg-warning bg-opacity-10 rounded">{{ $order->admin_note }}</div>
                    </div>
                    @endif
                </div>
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    @if ($payment)
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <span class="badge bg-{{ $payment->status->color() }} status-badge">
                                    {{ $payment->status->label() }}
                                </span>
                            </div>
                        </div>
                        ...
                        @if ($payment->requires_manual_verification)
                        <div class="mb-3">
                            <div class="info-label">Xác thực</div>
                            <div>
                                @if ($payment->is_verified)
                                    ...
                                    @if ($payment->verifier)
                                    <div class="small text-muted">
                                        <i class="fa-solid fa-user me-1"></i>Bởi: {{ $payment->verifier->name }}
                                    </div>
                                    <div class="small text-muted">
                                        <i class="fa-solid fa-clock me-1"></i>{{ $payment->verified_at->format('d/m/Y H:i') }}
                                    </div>
                                    @endif
                                @else
                                    <span class="badge bg-warning text-dark status-badge">
            ...
            <div class="card border-0 shadow-sm action-card">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        @if ($actions['canConfirmPayment'])
                            <button type="button" class="btn btn-success w-100" data-bs-toggle="modal" data-bs-target="#confirmPaymentModal">
                                <i class="fa-solid fa-check me-2"></i>Xác nhận thanh toán
                            </button>
                        @endif

                        @if ($actions['canRejectPayment'])
                            <button type="button" class="btn btn-danger w-100" data-bs-toggle="modal" data-bs-target="#rejectPaymentModal">
                                <i class="fa-solid fa-times me-2"></i>Từ chối thanh toán
                            </button>
                        @endif
