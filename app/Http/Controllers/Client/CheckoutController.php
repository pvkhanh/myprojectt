...
//         // Tạo order
//         $result = $this->checkoutService->createOrder($validated, $user);
...
//         // Nếu thanh toán bằng thẻ (Stripe)
//         if ($paymentMethod === PaymentMethod::Card) {
...
//                 return response()->json([
//                     'success' => false,
//                     'message' => $paymentIntent['message'] ?? 'Lỗi tạo thanh toán'
//                 ]);
//             }
...
//                 'payment' => [
//                     'id' => $payment->id,
//                     'gateway_response' => json_decode($payment->gateway_response),
//                     'client_secret' => $paymentIntent['client_secret'],
//                 ]
//             ]);
//         }

//         // Nếu COD hoặc Bank Transfer
//         return response()->json([
//             'success' => true,
//             'order' => $order,
//             'payment' => null
...
class CheckoutController extends Controller
{
    ...
    public function __construct(CheckoutService $checkoutService, StripeService $stripeService)
    {
        // Middleware auth
        // $this->middleware('auth');
        $user = auth()->user();
        if (!$user) {
            // redirect()->route('client.')->with('error', 'Giỏ hàng trống')->send();
            // exit;
        }
    ...
    public function index()
    {
        ...
        $validation = $this->checkoutService->validateCheckout($user);
        if (!$validation['valid']) {
            return redirect()->route('client.cart')->with('error', $validation['message']);
        }
