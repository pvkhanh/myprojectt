...
class BannerController extends Controller
{
    public function index(Request $request)
    {
        // CRITICAL: Load image relationship
        $query = Banner::with('image')->latest();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image_file')) {
            $file = $request->file('image_file');
            // Lưu vào folder banners trong storage/app/public
            $path = $file->store('banners', 'public');

            // Tạo record trong bảng images
            $image = Image::create([
                'path' => $path, // Lưu path như: banners/filename.jpg
                'type' => 'banner',
    ...
    public function bulkDelete(Request $request)
    {
        ...
        $banners = Banner::whereIn('id', $ids)->with('image')->get();
