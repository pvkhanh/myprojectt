...
// (chỉ cập nhật dòng is_active so sánh đúng enum)
// ---------------------------------------------------------------------------
...
class ProductResource extends JsonResource
{
    public function toArray($request): array
    {
        $statusEnum = $this->resource->status instanceof ProductStatus
            ? $this->resource->status
            : ProductStatus::fromString($this->resource->status);
        ...
        return [
            ...
            'in_stock' => $this->in_stock,
            'is_low_stock' => $this->is_low_stock,
            'is_active' => $statusEnum === ProductStatus::Active,
            'is_featured' => $this->is_featured ?? false,
            'status_label' => $statusEnum?->label() ?? null,
            'status_color' => $statusEnum?->color() ?? null,
            'category' => $this->categories->first() ? [
                'id' => $this->categories->first()->id,
                'name' => $this->categories->first()->name,
                'slug' => $this->categories->first()->slug,
